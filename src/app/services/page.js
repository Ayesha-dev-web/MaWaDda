          ...
          {services.map((service, index) => {
            ...
            return (
              <Link href={`/services/${service.slug}`} key={index}>
                <div className="p-6 text-center bg-white shadow-lg rounded-xl border border-gray-200 hover:shadow-2xl transition duration-300 cursor-pointer h-full flex flex-col">
                  {service.coverImage ? (
                    <div className="mb-4 w-full h-40 relative rounded-md overflow-hidden group">
                      <Image
